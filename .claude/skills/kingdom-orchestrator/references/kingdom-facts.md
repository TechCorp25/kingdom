...
| Automation layer | `.claude/` — `hooks/` (block-global-bash, block-global-knowledge, run-close-reminder, session-start), `settings.json`, `skills/` (automate-dev, handover). These enforce protections; do not fight them. |
