...
# Default targets (override by passing folder paths as arguments).
DEFAULT_SOURCES=(
  "$HOME/apps-original/Maps-Rebuild"
  "$HOME/app-work/MAPS-rebuild"
  "$HOME/app-work/maps-repo"
  "$HOME/app-work/maps-repo (1)"
  "$HOME/app-work/maps-repo-bup"
  "$HOME/civic-maps"
)
if [ "$#" -gt 0 ]; then
  SOURCES=("$@")
...
DEST_ROOT="${SNAPSHOT_DIR:-$HOME/kingdom/artifacts/snapshots}/$TS"
mkdir -p "$DEST_ROOT"
